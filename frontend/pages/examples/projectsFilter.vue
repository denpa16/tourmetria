    ...
    <div :class="['page', $style.ProjectsFilterPage]">
        <div class="container">
            ...
            >
                ...
                >
                    <div
                        v-show="view === 'list' && isReloadActive"
...
import { objectToQuery, applyQuery } from '~/assets/js/utils/query-utils';
...
export default {
    ...
    watch: {
        ...
        count(newCount, oldCount) {
            ...
            gsap.to(counterVals, {
                duration: .7,
                last: counterVals.next,
                onUpdate: () => {
                    this.counter = Math.ceil(counterVals.last);
                },
                ease: 'power1',
            });
        },
    ...
    methods: {
        ...
        handleUpdateQuery() {
            ...
            // Add query tags to filter
            if (Object.keys(this.$route.query).length) {
                const utmQuery = Object.fromEntries(Object.entries(this.$route.query)
                    .filter(([key]) => utmTags.includes(key)));
                values = { ...this.values, ...utmQuery };
            } else {
                values = { ...this.values };
            }

            const urlQuery = objectToQuery(values);
            window.history.replaceState(null, null, `${this.$route.path}${urlQuery?.length ? `?${urlQuery}` : ''}`);
        },
    },
};
</script>
