<template>
    ...
    >
        ...
        >
            <option
                v-for="(option, index) in optionList"
                :key="`${index}_mobile_${option.value}`"
                :value="option.value"
        ...
        optionList() {
            ...
            this.specs.forEach((opt, index) => {
                ...
                specs.push({
                    ...
                    selected: this.handleActiveNative(opt.value) ? index + 1 : false,
                });
            });
            ...
            if (this.multiple) {
                if (this.value) {
                    if (Array.isArray(this.value)) {
                        return this.specs.filter(a => this.value.includes(a.value));
                    } else {
                ...
                return this.specs.filter(a => a.value === this.value)[0] || null;
            }
        },
            ...
            if (this.multiple) {
                if (!option.value) {
                    ...
                    if (newValue.includes(option.value)) {
                        newValue.splice(this.value.indexOf(option.value), 1);
                    ...
                    if (newValue === option.value) {
                        ...
                        newValue = newValue.length ? [newValue, option.value] : option.value ? [option.value] : '';
                    }
                }
            } else {
                newValue = this.value !== option.value ? option.value : '';
            }
